...
func (c *Client) CreateGrafanaAPIKeyFromCloud(ctx context.Context, stack string, input *CreateAPIKeyRequest) (*CreateAPIKeyResponse, error) {
...
func (c *Client) CreateTemporaryStackGrafanaClient(ctx context.Context, stackSlug, tempKeyPrefix string, tempKeyDuration time.Duration) (tempClient *Client, cleanup func() error, err error) {
	stack, err := c.StackBySlug(ctx, stackSlug)
	if err != nil {
		return nil, nil, err
	}
	...
	apiKey, err := c.CreateGrafanaAPIKeyFromCloud(ctx, stackSlug, req)
	if err != nil {
		return nil, nil, err
	}
	...
	cleanup = func() error {
		_, err = client.DeleteAPIKey(ctx, apiKey.ID)
		return err
	}
